...
function _define_ip_address_pool {
    ...
    pkg_rev=$(porchctl rpkg clone -n default "https://github.com/isu-kim/catalog.git/distros/sandbox/metallb-sandbox-config@$REVISION" --repository mgmt-staging "$cluster-metallb-sandbox-config" | cut -f 1 -d ' ')
